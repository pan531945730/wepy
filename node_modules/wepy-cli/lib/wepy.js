...
var generateProject = function generateProject(name, config) {
    ...
    var copyFn = function copyFn(sourcePath) {
        return function (file) {
            var target = _path2.default.join(_util2.default.currentDir, file);
...
_commander2.default.option('-V', '显示版本号', function () {
    displayVersion();
});
_commander2.default.option('-s, --source <source>', '源码目录');
_commander2.default.option('-t, --target <target>', '生成代码目录');
_commander2.default.option('-f, --file <file>', '待编译wpy文件');
_commander2.default.option('-o, --output <type>', '编译类型：web，weapp。默认为weapp');
_commander2.default.option('-p, --platform <type>', '编译平台：browser, wechat，qq。默认为browser');
_commander2.default.option('--no-cache', '对于引用到的文件，即使无改动也会再次编译');
_commander2.default.option('--empty', '使用new生成项目时，生成空项目内容');
_commander2.default.option('--no-lint', '使用new生成项目时，禁用eslint');
_commander2.default.option('--redux', '使用new生成项目时，增加redux相关内容');
_commander2.default.option('-w, --watch', '监听文件改动');

_commander2.default.command('build').description('编译项目').action(function (projectPath) {

    if (_compile2.default.init(_commander2.default)) {
        _compile2.default.build(_commander2.default);
    }
});

_commander2.default.command('new <projectName>').description('生成项目').action(function (name) {
    generateProject(name || 'temp', _commander2.default);
});
