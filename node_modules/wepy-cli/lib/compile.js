...
exports.default = {
    findParents: function findParents(file) {
        ...
        files.forEach(function (f) {
            ...
            content && content.replace(/import\s*([{\w\d-_}]*)\s*from\s*['"](.*)['"]/ig, function (match, name, importpath) {
                reg = new RegExp('\\' + ext + '$');
                if (!reg.test(importpath)) importpath = importpath + ext;
    ...
    watch: function watch(config) {
        var _this3 = this;

        config.watch = false;

        var wepyrc = _util2.default.getConfig();
        var src = config.source || wepyrc.src || 'src';
        var dist = config.output || wepyrc.output || 'dist';
        _chokidar2.default.watch('.' + _path2.default.sep + src, wepyrc.watchOption || {}).on('all', function (evt, filepath) {
            if ((evt === 'change' || evt === 'add') && watchReady && !preventDup[filepath]) {
                ...
                config.file = _path2.default.join('..', filepath);
    ...
    wepyUpdate: function wepyUpdate() {
        var required = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : '1.6.0';
    ...
    build: function build(config) {
        ...
        var src = config.source || wepyrc.src;
        var dist = config.output || wepyrc.output;
        var ext = config.wpyExt || wepyrc.wpyExt;
        ...
        var file = config.output !== 'web' ? config.file : '';

        var current = process.cwd();
        var files = file ? [file] : _util2.default.getFiles(src);

        _cache2.default.setParams(config);
        ...
        if (config.output === 'web') {
            files.forEach(function (f, i) {
                if (i === 0) {
                    _index2.default.toWeb(f, config.platform || 'browser');
                } else {
